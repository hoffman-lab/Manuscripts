...
%% defaults
p = inputParser;
addRequired(p,'spikes',@isvector);
addRequired(p,'lfp',@isvector);
addParameter(p,'passband',[2 25],@isvector)
...
%         lfpphase = mod(lfpphase,2*pi);
        [wave,f,t,coh,wphases,raw,coi,scale,priod,scalef]=getWavelet(double(lfp),samplingRate,passband(1),passband(2),8,0);
...
if saveMat
 save('PhaseLockingData.cellinfo.mat','PhaseLockingData');
end
