function [index] = FitCCG(t,ccg,varargin)
...
% [~, id]  = findpeaks(smoothed); maxima = zeros(numel(t), 1); maxima(id) = 1; maxima = logical(maxima);
...
% [~, id]  = findpeaks(fit); maxima = zeros(numel(t), 1); maxima(id) = 1; maxima = logical(maxima);
peaksPos = t(t>0&maxima);
peaksNeg = t(t<0&maxima);
...
if strcmp(show,'on'),
	...
	hold on;
	bar(t,ccg);
    bar(t,ccg, 'FaceColor', [0 0 0], 'EdgeColor', [0 0 0]);
    set(gca, 'box', 'off', 'FontSize', 15, 'tickdir', 'out', 'LineWidth', 1.5, 'YTick', [], 'YColor', [1 1 1])
    
	plot(t,fit,'Color', [166 52 70]/255,'linewidth',5);
end
